...
//
// v1.8.0 — TTL-cached fetcher for platform JWKS endpoints.
//
// SCOPE
// ─────
// One narrow job: given a `jwks_uri`, return the decoded JWKS. Cache
// successful responses for `cacheTtlMs` so an LMS launch flurry
// doesn't hammer the LMS public-keys endpoint. Refuse non-HTTPS URLs
...
// in `jwt.ts` / `launch.ts`. The fetcher just transports bytes.
// ─────────────────────────────────────────────────────────────────────────────
