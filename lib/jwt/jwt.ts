...
// lib/jwt/jwt.ts
//
// v1.8.4 — Generic JWS / compact-JWT verifier (formerly `lib/lti/jwt.ts`).
//
// SCOPE
// ─────
...
// protocol-specific claim validation — that lives one layer up:
//
//   • `lib/lti/launch.ts`            — LTI 1.3 id_token claim checks
//   • `lib/district/oidc/id-token.ts` — OIDC ID token claim checks
//
// EXPIRATION HANDLING
// ───────────────────
...
export async function verifyJwt(
  ...
  const supportedAlg = alg as SupportedJwsAlgorithm;
